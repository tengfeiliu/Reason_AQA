...
class NegativeAttributionHead(nn.Module):
    """
    Negative Quality Attribution Head

    Takes temporal features and outputs a scalar temporal attribution signal [B, T]
    that identifies quality-degrading temporal segments.

    This is NOT attention - it's a temporal signal that models negative quality contribution.
    """
    def __init__(self, in_channels=64, hidden_dim=64, temporal_length=96):
        super(NegativeAttributionHead, self).__init__()
...
class NegativeQualityAggregator(nn.Module):
    """
    Aggregates negative quality contribution from temporal features
    using the attribution signal.
    """
    def __init__(self, feature_dim=64):
        super(NegativeQualityAggregator, self).__init__()

        # MLP to compute negative quality score from weighted features
    ...
    def forward(self, features, attribution):
        ...
        # Compute negative quality score
        negative_contribution = self.quality_mlp(aggregated)  # [B, 1]

        # Apply ReLU to ensure non-negative contribution
        negative_contribution = F.relu(negative_contribution)
