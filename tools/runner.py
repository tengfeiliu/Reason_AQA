...
def train_net(args):
    ...
    # Determine if dataset has segmentation labels
    has_segmentation = args.benchmark == 'FineDiving'
    args.has_segmentation = has_segmentation
    ...
    # build model
    base_model, psnet_model, decoder, regressor_delta, neg_attr_head, neg_quality_agg, pos_attr_head, pos_quality_agg = builder.model_builder(args)
    ...
    if use_gpu:
        ...
        if neg_attr_head is not None:
            neg_attr_head = neg_attr_head.cuda()
        if neg_quality_agg is not None:
            neg_quality_agg = neg_quality_agg.cuda()
        if pos_attr_head is not None:
            pos_attr_head = pos_attr_head.cuda()
        if pos_quality_agg is not None:
            pos_quality_agg = pos_quality_agg.cuda()
        torch.backends.cudnn.benchmark = True

    optimizer, scheduler = builder.build_opti_sche(base_model, psnet_model, decoder, regressor_delta, neg_attr_head, neg_quality_agg, pos_attr_head, pos_quality_agg, args)
    ...
    if args.resume:
        start_epoch, epoch_best_aqa, rho_best, L2_min, RL2_min = builder.resume_train(base_model, psnet_model, decoder,
                                                                                      regressor_delta, neg_attr_head, neg_quality_agg, pos_attr_head, pos_quality_agg, optimizer, args)
    ...
    if neg_attr_head is not None:
        neg_attr_head = nn.DataParallel(neg_attr_head)
    if neg_quality_agg is not None:
        neg_quality_agg = nn.DataParallel(neg_quality_agg)
    if pos_attr_head is not None:
        pos_attr_head = nn.DataParallel(pos_attr_head)
    if pos_quality_agg is not None:
        pos_quality_agg = nn.DataParallel(pos_quality_agg)
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        if neg_attr_head is not None:
            neg_attr_head.train()
        if neg_quality_agg is not None:
            neg_quality_agg.train()
        if pos_attr_head is not None:
            pos_attr_head.train()
        if pos_quality_agg is not None:
            pos_quality_agg.train()
        ...
        for idx, (data, target) in enumerate(train_dataloader):
            ...
            # forward
            if has_segmentation:
                # FineDiving: use PSNet and negative attribution
                label_1_tas = data['transits'].float().cuda() + 1
                label_2_tas = target['transits'].float().cuda() + 1
                helper.network_forward_train(base_model, psnet_model, decoder, regressor_delta, neg_attr_head, neg_quality_agg, pred_scores,
                ...
                # MTL-AQA, AQA-7, JIGSAWS: use positive attribution without PSNet
                helper.network_forward_train_no_segmentation(base_model, decoder, regressor_delta, pos_attr_head, pos_quality_agg, pred_scores,
                                                              video_1, label_1_score, video_2, label_2_score, mse, optimizer,
        ...
        validate(base_model, psnet_model, decoder, regressor_delta, neg_attr_head, neg_quality_agg, pos_attr_head, pos_quality_agg, test_dataloader, epoch, optimizer, args)
        helper.save_checkpoint(base_model, psnet_model, decoder, regressor_delta, neg_attr_head, neg_quality_agg, pos_attr_head, pos_quality_agg, optimizer, epoch,
...
def validate(base_model, psnet_model, decoder, regressor_delta, neg_attr_head, neg_quality_agg, pos_attr_head, pos_quality_agg, test_dataloader, epoch, optimizer, args):
    ...
    if neg_attr_head is not None:
        neg_attr_head.eval()
    if neg_quality_agg is not None:
        neg_quality_agg.eval()
    if pos_attr_head is not None:
        pos_attr_head.eval()
    if pos_quality_agg is not None:
        pos_quality_agg.eval()
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in enumerate(test_dataloader, 0):
            ...
            if has_segmentation:
                ...
                helper.network_forward_test(base_model, psnet_model, decoder, regressor_delta, neg_attr_head, neg_quality_agg, pred_scores,
                                            video_1, video_2_list, label_2_score_list,
                                            args, label_1_tas, label_2_tas_list,
                                            pred_tious_test_5, pred_tious_test_75)
            else:
                # MTL-AQA, AQA-7, JIGSAWS: use positive attribution without PSNet
                helper.network_forward_test_no_segmentation(base_model, decoder, regressor_delta, pos_attr_head, pos_quality_agg, pred_scores,
        ...
        if rho > rho_best:
            ...
            helper.save_checkpoint(base_model, psnet_model, decoder, regressor_delta, neg_attr_head, neg_quality_agg, pos_attr_head, pos_quality_agg, optimizer, epoch, epoch_best_aqa,
...
def test_net(args):
    ...
    # Determine if dataset has segmentation labels
    has_segmentation = args.benchmark == 'FineDiving_Pair_Dataset'
    args.has_segmentation = has_segmentation
    ...
    # load checkpoints
    builder.load_model(base_model, psnet_model, decoder, regressor_delta, neg_attr_head, neg_quality_agg, pos_attr_head, pos_quality_agg, args)
    ...
    test(base_model, psnet_model, decoder, regressor_delta, neg_attr_head, neg_quality_agg, pos_attr_head, pos_quality_agg, test_dataloader, args)

def test(base_model, psnet_model, decoder, regressor_delta, neg_attr_head, neg_quality_agg, pos_attr_head, pos_quality_agg, test_dataloader, args):
    global use_gpu
